...
power_data={
    ...
    'power_kwh':[100 + 50 * (i % 24)/24 + np.random.normal(0, 10) for i in range(1000)]
}

temp_data ={
    'timestamp':pd.date_range('2024-01-01', periods = 1000, freq='h'),
    'temperature':[25 + 10 * np.sin(2*np.pi*i/24) + np.random.normal(0,2) for i in range(1000)]
}


production_data ={
    'timestamp':pd.date_range('2024-01-01', periods = 1000, freq='h'),
    'production_rate':[np.random.poisson(50) for _ in range(1000)]
}
...
print(df.head())
print(f"\ntong so mau:{len(df)}")
print(f"cot du lieu: {df.columns.tolist()}")
...
print(f"pham vi binh thuong: {lower_bound:.2f} - {upper_bound:-2f} kwh")
